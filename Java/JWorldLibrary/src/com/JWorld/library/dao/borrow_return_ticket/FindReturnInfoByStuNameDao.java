 ...
 * @describe [查看所有的还书的记录]
 */
public class FindReturnInfoByStuNameDao {
    Connection conn;

    public FindReturnInfoByStuNameDao() {
        conn = new DBConnection().connection;
    }

    public List<ReturnInfo> FindReturnInfoByStuName(String stuName) {
        ArrayList<ReturnInfo> returnList = new ArrayList<>();
        String sql = "SELECT * FROM RETURN_INFO WHERE STUNAME = ?";
